...
const REMOVE_LABEL_FROM_NOTE = `${PREFIX}/removeLabelFromNote`;
...
export const addLabelToNote = createAppAsyncThunk(
  ...
  async (form: NoteLabelFormData, { fulfillWithValue, rejectWithValue }) => {
    const res = await fetch(`/api/notes/${form.noteId}/labels`, {
      method: "POST",
      headers: {
...
export const removeLabelFromNote = createAppAsyncThunk(
  ...
  extraReducers: (builder) => {
    ...
    builder
      ...
      .addMatcher(
        (action) =>
          isAnyOf(
            addLabelToNote.fulfilled,
            removeLabelFromNote.fulfilled,
          )(action),
        (state: NotesState, { payload }) => {
          state[payload.note.id] = payload.note;
        },
      );
  },
});
